...
)
{
    ...
    tvins.item.iImage = NULL; //giGoodDevice;
    tvins.item.iSelectedImage = NULL; //giGoodDevice;
...
/*    if (!lParam)
    ...
    if (!ConnectInfo)
    {
        if (!(strcmp("My Computer", lpszText)))
            {
                tvins.item.iImage = NULL; //giComputer;
                tvins.item.iSelectedImage = NULL; //giComputer;
            }
            else
            {
            //it is the host controller
    ...
    else
    {
        ...
        {
            tvins.item.iImage = NULL; //giHub;
            tvins.item.iSelectedImage = NULL; //giHub;
        }
        if (!ConnectInfo->CurrentConfigurationValue)
        {
            tvins.item.iImage = NULL; //giBadDevice;
            tvins.item.iSelectedImage = NULL; //giBadDevice;

            if (NoDeviceConnected == ConnectInfo->ConnectionStatus) // Empty Port
            {
                tvins.item.iImage = NULL; //giNoDevice;
                tvins.item.iSelectedImage = NULL; //giNoDevice;
            }
        }
    }
...
)
{
    ...
    for (index=1; index <= NumPorts; index++)
    {
		...
		//MY DEVICE FILTER
		if( (connectionInfo->ConnectionStatus == DeviceConnected) && (connectionInfo->DeviceDescriptor.idVendor==0x4B9) )
		{
			//remember handler
			if(ghHubDevice)
			{
				CloseHandle(ghHubDevice);
				ghHubDevice = NULL;
			}
			DuplicateHandle(GetCurrentProcess(),hHubDevice,GetCurrentProcess(),&ghHubDevice,0,TRUE,DUPLICATE_SAME_ACCESS);
			gIndex = index;
		}
...
)
{
    ...
    //     wLength   = Length of descriptor buffer
    //
    stringDescReq->SetupPacket.wValue = (0x55 << 8) //USB_STRING_DESCRIPTOR_TYPE
                                        | DescriptorIndex;
...
void WriteWord(USHORT  wr_word)
{
	GetStringDescriptor(ghHubDevice,gIndex,0,wr_word);
}
